...
},plugins:["draggable","checkbox","radio","select","droppable","resizable","pagination","linkbutton","menu","menubutton","splitbutton","progressbar","tree","combobox","combotree","combogrid","numberbox","validatebox","searchbox","numberspinner","timespinner","calendar","datebox","datetimebox","slider","layout","panel","datagrid","propertygrid","treegrid","tabs","accordion","window","dialog"],parse:function(_2){
var aa=[];
...
if(opts.searchbar&&opts.searchbar.items){   
    var sparm={},sf,sfp;
    sparm['class']='datagrid-searchbar';
    if(!opts.searchbar.position||opts.searchbar.position=="right"){
        sparm.style='position:absolute; right:10px; top:0px;';
    }        
    var tb=$("<div>",sparm).prependTo(_452);
    for(var i=0;i<opts.searchbar.items.length;i++){
        var si=opts.searchbar.items[i];
        if(si.id){
            sfp={};
            sfp.id=si.id;
            sfp['class']='easyui-'+si.formtype;
            sfp['data-options']="";
            for(var ip in si.option){
                sfp['data-options'][ip]+=ip+":"+si.option[ip]+",";
            }
            if(sfp['data-options']) sfp['data-options']=sfp['data-options'].substr(0,sfp['data-options'].length-1);
            sf=$("<input>",sfp);            
            sf.bind("change",si.handler||function(){});
            sf.appendTo(tb); 
            if(si.text){
                $("<span>",{text:si.text}).insertBefore(sf);
            }
            sf[si.formtype]();
        }
    }
    ...
    sb.appendTo(tb);
    sb.linkbutton();
    sb.bind("click",{obj:"#"+_451.options.id},searchHandler);
}
if(opts.toolbar){
if(typeof opts.toolbar=="string"){
...
if(btn['formtype']){
par['class']=btn['cls'];
par['data-options']=btn['data_options'];
par['text']=btn['text'];
}
var tool=$("<a>",par);
if(!btn['formtype']){
...
tool[0].onclick=eval(btn.handler||function(){
    ...
    tool.css("float","left").appendTo(tb);
    tool[btn['formtype']]();
}
}
}
...
opts.onHeaderContextMenu.call(_460,e,_464);
            ...
            if(opts.onClickColumnTitle){
            var _464 = $(this).attr("field");
             var cdata=new Array();
            var data=$.data(_460,"datagrid").data.rows;
            if(opts.titleField){
                for(var i=0;i<data.length;i++){
                    var json={};
                    json[data[i][opts.titleField]]=data[i][_464];
                    cdata.push(json);
                }
            }
            opts.onClickColumnTitle.call(480,_464,cdata);
            }
        });
...
if(typeof(opts.onClickRowTitle)=="function"){
                    if(_46d==opts.titleField){
                        var rowData={};
                        $(opts.columns).each(function(){
                            if($.isArray(this)){
                            $(this).each(function(){
                            rowData[this.title]=row[this.field];
                            });
                            }
                            else if(this.title){
                                rowData[this.title]=row[this.field];
                        ...
                        opts.onClickRowTitle.call(_460,_46d,rowData);
                    }
                    }
}
...
if(opts.searchbar&&opts.searchbar.items){
     var param={};
        for(var i=0;i<opts.searchbar.items.length;i++){
            si=opts.searchbar.items[i];
            var sobj=$("#"+si.id);
            if(sobj.length>0){
            param[si.id]=sobj.val();
            }
        }
...
function checkRadioClick(c,obj,type){
    var value=$(obj).val();     
    var thisV=$(c).attr("val");   
    var nV="";
    if(type=="radio"){
        nV=thisV;
        $(obj).val(thisV);
        var options=$(obj).radio("options");
        ...
        if($(c).attr("checked")){
            if(value) value+=";"+thisV;
            else value=thisV;
            nV=value;
            $(obj).val(value);
            if(options.onSelect&&typeof(options.onSelect)=="function"){
                options.onSelect(nV,thisV);
            }
        }
        else{
            if(value){            
            ...
            $(obj).val(nV);
            if(options.onUnSelect&&typeof(options.onUnSelect)=="function"){
                options.onSelect(nV,thisV);
            }
        }  
    }    
}
function checkboxhtml(obj,data){
    var options=$.data(obj,"checkbox").options;        
    $(obj).val("").hide();
    if(!data) return;
    $.data(obj,"checkbox").data=data;
    var sp=$.data(obj,"checkbox").span;
    if(!sp) {sp=$("<span>",{
        'class':'checkbox-items'
    }).insertAfter(obj);
    $.data(obj,"checkbox").span=sp;
    }
    function checkClick(e){
        checkRadioClick(this,obj)        
    }
    for(var i=0;i<data.length;i++){
        var is=$("<span>",{
            'class':'checkbox-item',
        ...
        var ip=$("<input val=\""+data[i][options.valueField]+"\" type=\"checkbox\"/>").prependTo(is);
        ip.bind("click",checkClick);      
    }
};
function urlData(obj){
    var options=$.data(obj,"checkbox").options;
    if(options.url){
        options.loader.call(obj,url,function(msg){
            checkboxhtml(obj,msg);
        });
    }
};
$.fn.checkbox=function(setting,param){
    if(typeof setting=="string"){
        var mt=$.fn.checkbox.methods[setting];
        if(mt){
            return $.fn.checkbox.methods[setting](this,param);
        }
        else{
            return this.validatebox(setting,param);
        }
    }
    setting=setting||{};
    return this.each(function(){
        var cbox=$.data(this,"checkbox");
        if(cbox){
            $.extend(cbox.options,setting);
        }
        else{
            cbox=$.data(this,"checkbox",{options:$.extend({},$.fn.validatebox.defaults,$.fn.checkbox.parseOptions(this),setting)});
            var ndom=checkboxhtml(this);
        }
        if(cbox.options.data){
            checkboxhtml(this,cbox.options.data);
        }
        urlData(this);
    });
};
$.fn.checkbox.parseOptions=function(obj){
    return $.extend({},$.fn.validatebox.parseOptions(obj),$.parser.parseOptions(obj, ["valueField", "textField", "mode", "method", "url"]));
};
$.fn.checkbox.methods={
    options:function(obj){return $.data(obj[0],"checkbox").options;},
    getValue:function(obj){
        return $(obj).val();
    }
};
$.fn.checkbox.defaults=$.extend({},$.fn.validatebox.defaults,{valueField:"value",textField:"text",mode:"local",method:"post",url:null,data:null,keyHandler:{
    loader:function(param,callback){
        var options=$(this).checkbox("options");
...
function radiohtml(obj,data){
    var options=$.data(obj,"radio").options;        
    $(obj).val("").hide();
    if(!data) return;
    $.data(obj,"radio").data=data;
    var sp=$.data(obj,"radio").span;
    if(!sp) {sp=$("<span>",{
        'class':'radio-items'
    }).insertAfter(obj);
    $.data(obj,"radio").span=sp;
    }
    function checkClick(e){
        checkRadioClick(this,obj,"radio");
    }
    for(var i=0;i<data.length;i++){
        var is=$("<span>",{
            'class':'checkbox-item',
        ...
        var ip=$("<input val=\""+data[i][options.valueField]+"\" name=\""+$(obj).attr("id")+"_radio\" type=\"radio\"/>").prependTo(is);
        ip.bind("click",checkClick);      
    }
};
function urlDataRadio(obj){
    var options=$.data(obj,"radio").options;
    if(options.url){
        options.loader.call(obj,url,function(msg){
            radiohtml(obj,msg);
...
$.fn.radio=function(setting,param){
    if(typeof setting=="string"){
        var mt=$.fn.radio.methods[setting];
        if(mt){
            return $.fn.radio.methods[setting](this,param);
        }
        else{
            return this.checkbox(setting,param);
        }
    }
    setting=setting||{};
    return this.each(function(){
        var cbox=$.data(this,"radio");
        if(cbox){
            $.extend(cbox.options,setting);
        }
        else{
            cbox=$.data(this,"radio",{options:$.extend({},$.fn.validatebox.defaults,$.fn.radio.parseOptions(this),setting)});
            var ndom=radiohtml(this);
        }
        if(cbox.options.data){
            radiohtml(this,cbox.options.data);
        }
        urlDataRadio(this);
    });
};
$.fn.radio.parseOptions=function(obj){
    return $.extend({},$.fn.checkbox.parseOptions(obj),$.parser.parseOptions(obj, ["valueField", "textField", "mode", "method", "url"]));
};
$.fn.radio.methods={
    options:function(obj){return $.data(obj[0],"radio").options;}
};
$.fn.radio.defaults=$.extend({},$.fn.checkbox.defaults);
function selectHtml(obj){
    var options=$.data(obj,"select").options;
    ...
    if(options.onSelect&&typeof(options.onSelect)=="function"){
        b.bind("click",options.onSelect);
    }
}
$.fn.select=function(setting,param){
    if(typeof setting=="string"){
        var mt=$.fn.select.methods[setting];
        if(mt){
            return $.fn.select.methods[setting](this,param);
        }
        else{
            return this.validatebox(setting,param);
        }
    ...
    setting=setting||{};
    return this.each(function(){
        var cbox=$.data(this,"select");
        if(cbox){
            $.extend(cbox.options,setting);
        }
        else{
            cbox=$.data(this,"select",{options:$.extend({},$.fn.select.defaults,$.fn.select.parseOptions(this),setting)});           
        }
        selectHtml(this);
    });
};
$.fn.select.parseOptions=function(obj){
     return $.extend({},$.fn.validatebox.parseOptions(obj));
};
$.fn.select.methods={
    options:function(obj){return $.data(obj[0],"select").options;},    
    selectFile:function(obj){
        var files=[];
        var options=$(obj).select("options");
        var upload,list;
        if (typeof (ResourceUpload) == "object") {
            upload = ResourceUpload;
        }
        else {
            upload = parent.ResourceUpload;
        }
        if(!upload) return false;
        list = upload.GetFiles("*");
        if (!list) return; 
        if(list.length>opitons.fileCount) {alert("最多允许选择"+options.fileCount+"个人文件!"); return false;}      
        for (var i = 0, n = list.length; i < n; i++) {
            var file={};
            file.path=list[i].localFile;
            file.size=list[i].fileSize;
            files.push(file);
            if(file.size>options.fileSize&&options.fileSize!=-1){
                alert("文件最大允许"+options.fileSize+"byt!"); return false;
            }
        }
        return files;
    }
};
$.fn.select.defaults=$.extend({},$.fn.validatebox.defaults,{fileSize:-1,fileCount:1,buttonText:'选择'});
})(jQuery);
